...
function sse(options) {
  ...
  function send(data, channelName) {
    ...
    console.info('Emitting event', channelWriteEventName);
  ...
  function middleware(req, res, next) {
     ...
     */
    function clean() {
      console.info('Cleaning', channelWriteEventName);
    ...
    if (shouldTreatAsEventStream) {
      flipToChannel();
      addChannelsToReq();
      console.info('Using write event name', channelWriteEventName);

      // Turnover if the connection threshold is exceeded

      if (options.turnover && conns.push(res) > maxconn) {
        console.info('Turnover', options.turnover, conns.push(res) > maxconn);
